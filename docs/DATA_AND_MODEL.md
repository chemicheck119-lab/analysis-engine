...
차아염소산나트륨–염산과 금속 나트륨–염산 2쌍이며, 나머지 공개 검증 13쌍은 CAMEO
단일체계 상태입니다. 이 registry는 현재 재고·실제 혼합·현장 농도나 사람 전문가 승인을
증명하지 않습니다.
...
`CAS_EVIDENCE_NOT_LOADED`로 남깁니다.

### 8.1 후보 재순위화와 다음 확인 정책

`material-evidence-ranker-v1`은 다음 여섯 특징을 사용합니다.

1. 기존 정확검색·FTS5 BM25 순위 사전값
2. Resolver 문자열 유사도
3. 정확 CAS·별칭 일치 여부
4. 별칭 출처의 공개기관·검증 단계
5. 공개 물성 일치 필드 비율
6. 동일 CAS 공식 근거 적재 여부

검수된 현장 질의-CAS 라벨이 부족하므로 지도학습을 실행하지 않았고, 버전 고정 선형 정책을
사용합니다. 점수는 후보 정렬값이며 물질 정답 확률·위험 확률·현장 확인을 의미하지 않습니다.
후보가 여러 개면 정보이득으로 다음 구분 확인을 제안하지만 최종 CAS 확인은 용기 라벨·현장
MSDS·운송 문서 등 기존 confirmation gate가 담당합니다.
...
- `official_national_incident_parser_locked_2026-08-01.json`: 화학물질안전원 전국 사고
  2021~2025년 442건의 비식별 집계 파서 외부 감사
- `e2e_scenarios_draft.jsonl`: 실제 사고 분석 경로의 확인 gate·기권·충돌 규칙 상태 전이
- `material_ranker_self_retrieval_2026-08-01.json`: 749개 공개 물성 프로필 중 300건
  자기검색 회귀 감사 결과
...
| 미확인 상태 Rule 실행 | 8 | 0건 |
| 미확인 위험 결과 노출 | 8 | 0건 |
| 물성 자기검색 BM25·Ranker Top-1 | 300 | 0.6933 → 0.6933 |
| 물성 자기검색 BM25·Ranker Top-3 | 300 | 0.8000 → 0.8000 |
| 구분 가능한 다음 확인 생성 | 300 | 0.7733 |
| 전국 공식 사고유형 Recall | 442건 중 유효 라벨 431건 | 0.8376 |
| 전국 사고문 관찰 가능 물질명 Recall | 319 라벨 | 0.8150 |

21·10·12·8건과 300건 자기검색은 내부 회귀 지표입니다. 전국 공식 사고 442건은 기관·시간을
분리한 외부 보류 감사지만 CAS 정답과 실제 신고 음성 전사가 없습니다. 어느 결과도 현장
정확도, 전체 물질 성능, 사고 대응 성공률로 인용하면 안 됩니다. 지표 정의와 실패 분석은
[모델 평가](EVALUATION.md)를 참고하세요.
...
전국 사고 개발 구간에서 확인한 직접 방출 표현(`누설`, `비산`, `분출` 등)과 정체성을
바꾸지 않는 제한된 결합 표현(`염소가스`, `질산용기`)은 인식합니다. `염소소독제`,
`차아염소산`, `폐염산`처럼 다른 제품·화합물·폐기물일 수 있는 포함 표현은 염소·염산 CAS로
승격하지 않습니다.

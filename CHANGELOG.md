...
- 서울·인천에서 동일 왜곡 조건·동일 저성능 신호가 반복되고 후단 Gate가 모두 통과할 때만
  제한된 Whisper LoRA 실험 설계 후보로 넘기는 교차지역 진입 Gate
...
- 2019 검증 353건과 2020 잠금 419건에서 카탈로그 포함률·Top-1·Top-3·MRR·미관측 성능을
  함께 비교하고, 2020 Top-1 0.8974·Top-3 0.9021·잘못된 단일 exact 0건을 기록한 평가 스냅샷

- 화학물질안전원 전국 화학사고 1,026건을 2014~2020 개발 584건과 2021~2025 잠금 442건으로
  분리하고 원문·업체·주소를 남기지 않는 공식 외부 파서 평가기와 고정 checksum 수집기
- 전국 미래 보류 442건에서 사고유형 Recall 0.8376, 관찰 가능 물질명 Recall 0.8150,
  미확인 후보 Rule 입력 0건을 기록한 비식별 집계 평가 스냅샷
- 직접적인 누설·비산·분출 표현과 안전한 `물질명+가스/용기/탱크` 경계를 추가하고,
  포함 제품명·부정 사고 표현은 승격하지 않는 파서 회귀 테스트
- 전국 외부 보류평가와 v3 Resolver가 포함된 main을 서울 Cloud Run preview에 무중단
  배포하고, 잘못된 DB 후보를 readiness가 트래픽 전환 전에 차단한 실제 운영 증빙
...
- 공식 소방 사고 원천에 유효 CAS와 물질명이 있어도 기본 카탈로그 밖이라는 이유만으로
  학습 표현 272건을 제외해 반복 사고 물질 후보를 찾지 못하던 문제
- source-only CAS가 문자 유사도만으로 다른 물질의 fuzzy 후보가 되거나 재학습 시 일반
  카탈로그 물질로 승격될 수 있던 안전 경계
- 전체 2015~2020 기간의 모호 표현을 먼저 계산해 2018 학습 필터가 미래 연도 정보를
  간접적으로 보던 시간 분할 누수 가능성
